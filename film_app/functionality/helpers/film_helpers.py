...
def movie_extractor(movie_request):
    "Uses Cinemagoer object to extract selected film"
    iterator = 0
    cine_obj = Cinemagoer()
    while iterator < 10:
        try:
            ...
            iterator += 1
            print(
                f"Error with cinemagoer extraction, retrying extraction: try no. {iterator}, error detail: {e}"
            )


def movie_maker(db: Session, request=schemas.FilmRequest):
    "Extracts film details and commits to db"
    retrieved_film = movie_extractor(request)
    new_movie = models.Film(
        name=retrieved_film["name"],
        release_year=retrieved_film["release_year"],
        user_id=retrieved_film["user_id"],
        imdb_id=retrieved_film["imdb_id"],
    )
    db.add(new_movie)
    db.commit()
...
def film_deleter(id: int, db: Session):
    ...
    if not film.first():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"film with id {id} not found"
        )
